-- boot.adb - Pure Ada Bootloader with Correct Inline Assembly
-- Properly formatted for Ada compiler
with Interfaces;
...
procedure Boot is
   ...
   -- ==============================
   -- GDT SETUP (Pure Ada with Correct Assembly)
   -- ==============================
   ...
   type GDT_Entry is record
      ...
      Base_Low   : Word;
      Base_Mid   : Byte;
      Access_Bit : Byte;  -- Renamed from "Access" (reserved word)
      Granularity: Byte;
      Base_High  : Byte;
   end record;
   ...
   GDT : GDT_Table := (
      ...
      (Limit_Low  => 16#FFFF#, -- 4GB limit
       Base_Low   => 0,
       Base_Mid   => 0,
       Access_Bit => 16#9A#, -- Code segment, present, readable, executable
       Granularity=> 16#CF#, -- 4KB granularity, 32-bit
       Base_High  => 0),
      (Limit_Low  => 16#FFFF#, -- 4GB limit
       Base_Low   => 0,
       Base_Mid   => 0,
       Access_Bit => 16#92#, -- Data segment, present, writable
       Granularity=> 16#CF#, -- 4KB granularity, 32-bit
       Base_High  => 0),
      (Limit_Low  => 0,
       Base_Low   => 0,
   ...
   begin
      ...
      -- Direct machine code approach for GDT setup
      Asm ("lgdt (%0)", 
           Inputs => GDT_Desc'Asm_Input ("a", GDT_Desc),
           Clobber => "memory");
   end Setup_GDT;
   
   -- ==============================
   -- ENTER PROTECTED MODE (Pure Ada with Correct Assembly)
   -- ==============================
   ...
   begin
      -- Using direct calls instead of inline assembly for simplicity
      -- This will be replaced with actual protected mode code
      null;
...
begin
   ...
   -- Call EmergeOS directly (simplified)
   EmergeOS;
